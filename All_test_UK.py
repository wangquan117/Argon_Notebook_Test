...
def run_key_board():
    ...
    try:
        ...
        subprocess.run(["python3", "argon-scripts/Argon_Notebook_Test-main/Key_Board_UK.py"], check=True)
...
def run_recording_playback():
    ...
    try:
        ...
        if not os.path.exists(music_path):
            return f"run_recording_playback____NO: Music file not found at {music_path}"
        subprocess.run(["timeout", "10", "ffplay", "-nodisp", "-autoexit", music_path], check=True)
