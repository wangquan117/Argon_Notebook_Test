...
已生成: D:\downlo\arcade-Avoid-the-Fans-2-st7789.uf2 (约 2097152 bytes)
  ili9341_init: patched 1 site(s) ...
  palette: patched 1 site(s) at 0x10013xxx; LUT at ...
  cf2: wrote Kubit CF2 at 0x100FF000
已按 RP2040-E14 连续填充到 0x10100000
```

UF2 磁盘大小约为 Flash 写入量的 **两倍**（每 256 字节数据占 512 字节 UF2 块）。默认只填到 **1MB CF2 槽**（约 **2MB** 文件），不是整颗 2MB 芯片（那会变成 4MB）。Arcade 从 1MB 槽开始找 CF2，找到就停。

不能再缩到跟原版游戏一样小，同时还带上 CF2：游戏在 Flash 开头，CF2 必须在 `0x100FF000`，中间必须用 `0xFF` 填满，否则 RP2040-E14 会黑屏。板子上**已经有一份可用 CF2** 时，可以：

```powershell
py -3 "D:\downlo\patch_uf2.py" "D:\downlo\arcade-Save-the-Baby-Dinos-2.uf2" --skip-cf2
```

这样只连续填充游戏+调色板 LUT，体积接近原版。

旧的稀疏 632KB 补丁会黑屏；不要用 `--no-e14-pad`。

复制到 `RPI-RP2` 请等盘符自己消失再拔线。
...
| 补丁成功但板子全黑 | 必须用带 E14 填充的脚本（默认约 **2MB**）。旧的 632KB 稀疏 `*-st7789.uf2` 有 Flash 空洞。不要用 `--no-e14-pad`。 |
